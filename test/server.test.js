...
test('content pages redirect their .md form to the clean URL', async () => {
  ...
  assert.equal(new URL(res.headers.get('location'), base).pathname, '/getting-started')
...
test('trailing slashes redirect to the canonical clean URL', async () => {
  ...
  assert.equal(new URL(dir.headers.get('location'), base).pathname, '/blog')
