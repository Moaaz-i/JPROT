...
inheritance puzzle — see [Customization](customization).

## How do I add a blog?

Create `content/blog.md` (with `layout: blog`) and start adding Markdown files
under `content/blog/`. See [Customization → Blog](customization#7-blog).

## Does it have dark mode?

Yes, a built-in light/dark toggle is in the header, and it follows your OS
preference by default. See [Customization → Dark mode](customization#4-dark-mode).

## Can I use my own components?

Absolutely. Components are plain JS functions returning an HTML string. Drop
replacements into `theme/components/`. See [Customization](customization).
